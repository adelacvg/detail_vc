...
if __name__ == "__main__":
    ...
    parser.add_argument("--in_dir", type=str, default="./datasets", help="path to source dir")
    parser.add_argument("--out_dir2", type=str, default="./44k", help="path to target dir")
    parser.add_argument("--skip_loudnorm", action="store_true", help="Skip loudness matching if you have done it")
    args = parser.parse_args()

    print(f"CPU count: {cpu_count()}")
    dirs = glob(os.path.join(args.in_dir, "**", ""), recursive = True)
    speakers = [dir_ for dir_ in dirs if is_speaker(dir_)]
    cnt=0
